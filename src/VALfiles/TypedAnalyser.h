...
#ifdef STATICHACK
extern bool overruleStaticInformation;
extern void setStatics(const set<string> & s);
extern int findOutIfStatic(const extended_pred_symbol * e);
#endif

/**
 *  A class representing a predicate symbol, with a given combination of typed
 *  parameters.  TIM creates a <code>extended_pred_symbol</code> for each combination of types
 *  and sub-types that can be passed to a given predicate.  For instance, if
 *  we have the PDDL type structure <code>(tea toast - item)</code>, and a predicate
 *  <code>(cooked ?i - item)</code>, then three <code>extended_pred_symbol</code>s are
 *  created for the <code>cooked</code> predicate:
 *   - one where the type of the item was typed in the problem definition as <code>tea</code>
 *   - one where the type of the item was typed in the problem definition as <code>toast</code>
 *   - one where the type of the item was typed in the problem definition as <code>item</code>
 *
 *  Each of these three has a common parent, a <code>holding_pred_symbol</code>, which can be obtained by
 *  using the <code>getParent</code> function.
 *
 *  Because multiple <code>extended_pred_symbol</code>s are created for each PDDL predicate, care must be taken
 *  when interpreting the results of methods such as <code>appearsStatic()</code> - this will, for instance, report
 *  whether a given typed variant of the predicate is static, not necessarily whether the base PDDL predicate
 *  is static.  To ascertain properties of the base predicate as a whole, use <code>getParent()</code> then
 *  with this iterate through all the derived <code>extended_pred_symbol</code>s.
 *
 *  @see holding_pred_symbol
 */
...
protected:
    /**
     *  The predicate parent of this object, common amongst all <code>extended_pred_symbol</code>s
     *  derived from the same base PDDL predicate.
     */
...
//			cout << "\n";
     ...
     *  that can be used with that predicate in the domain and problem definition.  The
     *  parent returned by this function is identical for all the <code>extended_pred_symbol</code>s
     *  derived from the same PDDL predicate.
     *
     *  @return The parent of the current object, common amongst all predicates with this object's name.
     */
	holding_pred_symbol * getParent() const {
        return parent;
    }
    /**
     *  Specify the parent of the current object.  The use of type inheritance introduces multiple
     *  <code>extended_pred_symbol</code>s, one for each combination of types and sub-types
     *  that can be used with that predicate in the domain and problem definition.
     *
     *  @param h The parent of the current <code>extended_pred_symbol</code>.  This should be common
     *           amongst all predicates derived from the same PDDL predicate as this one (i.e. with
     *           the same name).
     */
...
// into the timed initial literals!
	...
	bool isStatic() const 
	{
        ...
        if (overruleStaticInformation) {
            if (overruledStaticStatus == -1) {
                overruledStaticStatus = findOutIfStatic(this);
            }
            return (overruledStaticStatus == 1);
        } else {
...
/**
 *  An extension of the <code>pred_symbol</code> class, used by TIM to coordinate
 *  typed domain analysis.  <code>holding_pred_symbol</code>s are substituted in when parsing
 *  predicates in the domain and problem files.
 *
 *  Each <code>holding_pred_symbol</code> has one or more derived <code>extended_pred_symbol</code>
 *  objects - one for each combination of types and sub-types that can be passed to the predicate.
 *  Following the TIM analysis, these can be used to ascertain domain properties.
 *
 *  @see extended_pred_symbol, see TIM::performTIMAnalysis
 */
...
private:
    ...
    /**
      ...
      *  predicate represented by this object.
      */
	Preds preds;
...
public:
			...
			if(filterFn(p->getProp()->head))
			{
				if(pos)
				{
					if (op) {
                        ...
                        #ifdef TADEBUG
                        cout << "Recording precondition on " << p->getProp()->head->getName() << std::endl;
                        #endif
                    }
		...
		else
		{
			if(filterFn(p->prop->head))
			{
				if(adding)
				{
                    ...
                    #ifdef TADEBUG
                    cout << "Recording add of " << p->prop->head->getName() << " at " << EPS(p->prop->head) << std::endl;
                ...
                cout << "Effect on " << p->prop->head->getName() << " at " << EPS(p->prop->head) << " has been filtered\n";
            };
		};
	};
	virtual void visit_forall_effect(forall_effect * p) 
	...
	virtual void visit_operator_(operator_ * p) 
	{
        ...
        p->precondition->visit(this);
        #ifdef TADEBUG
        cout << "Visiting effects of " << p->name->getName() << std::endl;
        #endif
		p->effects->visit(this);
		op = 0;
	};
