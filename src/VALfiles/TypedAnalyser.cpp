...
#ifdef STATICHACK
bool overruleStaticInformation = false;
set<string> staticNames;

void setStatics(const set<string> & s)
{
    staticNames = s;
    overruleStaticInformation = true;
};

int findOutIfStatic(const extended_pred_symbol * e)
{
    if (staticNames.find(e->getName()) != staticNames.end()) {
        cout << "HACK: Responding that " << e->getName() << " is static\n";
        return 1;
    }
    return 0;
}
