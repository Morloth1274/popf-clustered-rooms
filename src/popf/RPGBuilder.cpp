...
public:
    ...
    GoalNumericCollector(list<RPGBuilder::NumericPrecondition> * ng, list<Literal*> * g, map<string, int> & p, const VAL::operator_ * o, FastEnvironment * f, VAL::TypeChecker * t = 0) :
            numericGoals(ng), literalGoals(g), numToUse(numericGoals), litToUse(literalGoals), prefNameToID(p), tc(t), adding(true), op(o), fe(f), inpres(true), checkpos(true), inConstraints(false), inPreference(false) {};
...
public:
    ...
    TimedPrecEffCollector(instantiatedOp* currIOP, const VAL::operator_ * o, FastEnvironment * f, VAL::TypeChecker * t = 0) :
            ...
            addToListNumeric(0), addEffToListNumeric(0), isDurative(false) {
...
// it quantifies.
    ...
    virtual void visit_cond_effect(cond_effect * p) {
        ...
        if (!isDurative) {
            ...
            addEffToListNumeric = &(condEffs.back()->startNumericEff);
        } else {
    ...
    virtual void visit_timed_effect(timed_effect * p) {
        ...
        case VAL::E_AT_END: {
            ...
            if (visitingWhen) {
                ...
                addEffToListNumeric = &(condEffs.back()->endNumericEff);
            } else {
...
//      case VAL::E_AT_START:
        ...
        /*
                default:
                    cout << "Error, unsupported effect time specification in action schema (not start or end)\n";
                    exit(0);
                    break;*/
        };
        p->effs->visit(this);
    ...
    virtual void visit_assignment(assignment * a) {
        ...
        if (!addEffToListNumeric) {
            startNumericEff.push_back(RPGBuilder::NumericEffect(a->getOp(), pne->getID(), const_cast<VAL::expression*>(a->getExpr()), fe, tc));
            bool isCTS = false;
            ...
            addEffToListNumeric->push_back(RPGBuilder::NumericEffect(a->getOp(), pne->getID(), const_cast<VAL::expression*>(a->getExpr()), fe, tc));
        }
    };
...
public:
    ...
    InitialStateCollector(const VAL::operator_ * o, FastEnvironment * f, VAL::TypeChecker * t = 0) :
        ...
        const int ifSize = instantiatedOp::howManyPNEs();
        initialFluents = vector<double>(ifSize);
        for (int i = 0; i < ifSize; ++i) {
            initialFluents[i] = 0.0;
        }
    };
...
vector<bool> RPGBuilder::startEndSkip;
...
RPGBuilder::Metric * RPGBuilder::theMetric;
...
bool checkIfRogue(TimedPrecEffCollector & c)
{
...

    {
        ...
        for (; llItr != llEnd; ++llItr) {
            const int tmpID = (*llItr ? (*llItr)->getID() : -1);
            if (tmpID < 0) {
...
void RPGBuilder::initialise()
{
    ...
    const int literalCount = instantiatedOp::howManyLiterals();
    const int pneCount = instantiatedOp::howManyPNEs();
        ...
        for (int i = 0; tilItr != tilEnd; ++tilItr, ++i) {
...

            {
                ...
                for (; elItr != elEnd; ++elItr) {
                    effectsToActions[(*elItr)->getID()].push_back(make_pair(i, VAL::E_AT));
                }
            }
            {
                ...
                for (; elItr != elEnd; ++elItr) {
                    negativeEffectsToActions[(*elItr)->getID()].push_back(make_pair(i, VAL::E_AT));
                }
            }
        }
...
//  for (int i = 0; i < pneCount; ++i) increasedReset[i] = pair<int, double>(-1, 0.0);

    {
        ...
        for (int i = 0; lsItr != lsEnd; ++lsItr, ++i) {
            literals[i] = *lsItr;
            //cout << "Literal " << i << " - " << *(*lsItr) << " with ID " << (*lsItr)->getID() << "\n";
        }
    }
...

    {
        ...
        PNEStore::iterator pneEnd = instantiatedOp::pnesEnd();
        for (; pneItr != pneEnd; ++pneItr) {
            pnes[(*pneItr)->getID()] = *pneItr;
            if (PNEdebug) cout << *(*pneItr) << "\n";
        }
    ...
    {
        GoalNumericCollector c(&numericGoals, &literalGoals, prefNameToID, 0, 0, theTC);
        current_analysis->the_problem->visit(&c);
    ...
    if (doTemporalAnalysis) {
        ...
        TemporalAnalysis::dummyAnalysis();
    }
...
bool RPGBuilder::stepNeedsToHaveFinished(const ActionSegment & act, const MinimalState & theState, set<int> & toBeNonMutex)
{
    ...
    const map<int, PropositionAnnotation>::const_iterator retiredStateEnd = theState.retired.end();
    ...
    if (willDelete) {
        ...
        for (; fItr != fEnd; ++fItr) {
            const map<int, PropositionAnnotation>::const_iterator invItr = theState.first.find((*fItr)->getID());
            if (invItr != stateEnd) {
    ...
    if (willAdd) {
        ...
        for (; fItr != fEnd; ++fItr) {
            const map<int, PropositionAnnotation>::const_iterator invItr = theState.retired.find((*fItr)->getID());
...
double RPGBuilder::DurationExpr::minOf(const vector<double> & minFluents, const vector<double> & maxFluents)
{
    ...
    for (int i = 0; i < lim; ++i) {
        ...
        if (currW < 0.0) {
            toReturn += currW * maxFluents[variables[i]];
        } else {
            toReturn += currW * minFluents[variables[i]];
        }
    }
...
double RPGBuilder::DurationExpr::maxOf(const vector<double> & minFluents, const vector<double> & maxFluents)
{
    ...
    for (int i = 0; i < lim; ++i) {
        ...
        if (currW < 0.0) {
            if (debug) {
                cout << " - " << -currW << "x" << minFluents[variables[i]];
            }
            toReturn += currW * minFluents[variables[i]];
        } else {
            if (debug) {
                cout << " + " << currW << "x" << maxFluents[variables[i]];
            }
            toReturn += currW * maxFluents[variables[i]];
        }
    }
        ...
        for (; ceItr != ceEnd; ++ceItr) {
            ...
            for (int pass = 0; pass < 3; ++pass) {
                ...
                for (; clItr != clEnd; ++clItr) {
                    ...
                    list<pair<int, VAL::time_spec> > & addedBy = effectsToActions[(*clItr)->getID()];
                        ...
                        for (int pol = 0; pol < 2; ++pol) {
                            ...
                            for (; eItr != eEnd; ++eItr) {
                                ...
                                if (oItr == oEnd) {
                                    ...
                                    if (pol) {
                                        negativeEffectsToActions[(*eItr)->getID()].push_back(make_pair(i, VAL::E_AT_START));
                                    } else {
                                        effectsToActions[(*eItr)->getID()].push_back(make_pair(i, VAL::E_AT_START));
                                    }
                                }
                                if (pol) {
                                    negativeEffectsToActions[(*eItr)->getID()].remove(make_pair(i, VAL::E_AT_END));
                                } else {
                                    effectsToActions[(*eItr)->getID()].remove(make_pair(i, VAL::E_AT_END));
                                }
                            }
                            currList.clear();
                        }
...
list<RPGBuilder::DurationExpr *> RPGBuilder::buildDEList(list<RPGBuilder::NumericPrecondition *> & d)
{
    ...
    for (; lItr != lEnd; ++lItr) toReturn.push_back(buildDE(*lItr));
    return toReturn;
};
...
//  cout << "Building duration expression for " << *expr << "\n";

    pair<list<double>, list<int> > result;

    WhereAreWeNow = PARSE_DURATION;
    makeWeightedSum(expr->RHSformula, result);
    WhereAreWeNow = PARSE_DURATION;

    int rSize = result.first.size();

//  cout << "Final expression has " << rSize << " terms\n";
